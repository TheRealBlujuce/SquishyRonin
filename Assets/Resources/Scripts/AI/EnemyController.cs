...
public class EnemyController : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
			if (katanna.canCut) { DieFromKatanna(katanna); }
        }
    }
    ...
    private IEnumerator PerformAttack()
{
	...
	private void MeleeAttackCalculation()
	{
		...
		if (dist >= attackRange)
		{
			...
			moveDirection = GetObstacleAvoidanceDirection(desiredDir);
	...
	{
		float checkDistance = 0.5f; // How far ahead to check for obstacles
		RaycastHit2D hit = Physics2D.Raycast(transform.position, desiredDirection, checkDistance, obstacleLayer);
		if (hit.collider != null)
		{
			// Obstacle detected: try to slide around it by picking a perpendicular direction
			Vector2 avoidDirection = Vector2.Perpendicular(desiredDirection);

			// Check which side is more free
			RaycastHit2D leftCheck = Physics2D.Raycast(transform.position, avoidDirection, checkDistance, obstacleLayer);
			if (leftCheck.collider != null)
				avoidDirection = -avoidDirection; // try the other side

			// Optionally: smooth turn towards avoidDirection
			Vector2 newDir = Vector2.Lerp(desiredDirection, avoidDirection, 0.75f).normalized;
			return newDir;
		}
		else
		{
			// No obstacle ahead, move as desired
...
#region Notes and Code Snippits
...
//if (!isAttacking && !isShooting && !playerObject.isDead) {
...
//	{
//		Vector2 target = currentPath[0].worldPosition;
